...
#define PARSE_MACADDR(len, format) \
if (count != len)  \
	count = sscanf(str, format, &a, &b, &c, &d, &e, &f, junk);

#define PARSE_MACADDR8(len, format) \
if (count != len)  \
...
sqlite_fdw_macaddr_str(sqlite3_context* context, int argc, sqlite3_value** argv)
{
	...
	if (len == MACADDR_LEN )
	{
		char	   *result = (char *) palloc(len * 4);
		sqlite3_int64	value = sqlite3_value_int64(arg);
		snprintf(result, len * 4, "%02x:%02x:%02x:%02x:%02x:%02x",
	...
	if (len == MACADDR8_LEN )
	{
		char	   *result = (char *) palloc(len * 4);
		sqlite3_int64	value = sqlite3_value_int64(arg);
		snprintf(result, len * 4, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
...
sqlite_fdw_macaddr6_int (const unsigned char* s, sqlite_uint64*	i)
{
	 ...
	 PARSE_MACADDR(MACADDR_LEN, "%x:%x:%x:%x:%x:%x%1s");
	 PARSE_MACADDR(MACADDR_LEN, "%x-%x-%x-%x-%x-%x%1s");
	 PARSE_MACADDR(MACADDR_LEN, "%2x%2x%2x:%2x%2x%2x%1s");
	 PARSE_MACADDR(MACADDR_LEN, "%2x%2x%2x-%2x%2x%2x%1s");
	 PARSE_MACADDR(MACADDR_LEN, "%2x%2x.%2x%2x.%2x%2x%1s");
	 PARSE_MACADDR(MACADDR_LEN, "%2x%2x-%2x%2x-%2x%2x%1s");
	 PARSE_MACADDR(MACADDR_LEN, "%2x%2x%2x%2x%2x%2x%1s");
...
sqlite_fdw_macaddr8_int (const unsigned char* s, sqlite_uint64*	i)
{
	 ...
	 /* %1s matches if there is trailing non-whitespace garbage */
 	 PARSE_MACADDR8(MACADDR8_LEN, "%x:%x:%x:%x:%x:%x:%x:%x%1s");
	 PARSE_MACADDR8(MACADDR8_LEN, "%x-%x-%x-%x-%x-%x-%x-%x%1s");
	 PARSE_MACADDR8(MACADDR8_LEN, "%2x%2x%2x%2x:%2x%2x%2x%2x%1s");
	 PARSE_MACADDR8(MACADDR8_LEN, "%2x%2x%2x%2x-%2x%2x%2x%2x%1s");
	 PARSE_MACADDR8(MACADDR8_LEN, "%2x%2x.%2x%2x.%2x%2x.%2x%2x%1s");
	 PARSE_MACADDR8(MACADDR8_LEN, "%2x%2x-%2x%2x-%2x%2x-%2x%2x%1s");
	 PARSE_MACADDR8(MACADDR8_LEN, "%2x%2x%2x%2x%2x%2x%2x%2x%1s");
